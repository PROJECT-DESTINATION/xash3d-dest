...
def options(opt):
	...
	grp.add_option('--gamedir', action = 'store', dest = 'GAMEDIR', default = 'destination',
...
def configure(conf):
	...
	conf.env.DISABLE_LAUNCHER = conf.env.DEST_OS in ['android', 'nswitch', 'psvita', 'dos', 'ps3'] or conf.env.MAGX
